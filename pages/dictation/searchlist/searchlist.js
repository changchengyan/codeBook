 ...
 */
  ...
  loadBookList: function () {
    ...
    if (that.data.loadMore && !that.data.loadding) {
      ...
      app.Dictation.GetDictationBookList
        (
        function (res) {
          console.log(res);
          var data = res.data;
          ...
          if (res.data.list.length > 0) {
            // 追加数据
            var loadLastId = Number(data.list[data.list.length - 1].rownumber) + 1;
            ...
            for (var i = 0; i < res.data.list.length; i++) {
              for (var j = 0; j < that.data.gradeArr.length; j++) {
                if (res.data.list[i].book_grade == that.data.gradeArr[j]) {
                  if (that.data.bookJson[that.data.gradeArr[j]]) {
                    that.data.bookJson[that.data.gradeArr[j]].push(res.data.list[i]);
                  } else {
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: "年级分类",
      path: '/pages/dictation/searchlist/searchlist?adviser_id=' + adviser_id + '&share=true'
    }
  },
